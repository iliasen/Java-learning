...
import org.iliasen.daoConection.model.Student;
...
public class StudentDao {
    ...
    public int insert(Student student) {
        ...
        try {
            String query = "insert into student(fname,lname,address,mobile_no,email_id,city,designation,dob,doj,salary) " + "values(?,?,?,?,?,?,?,?,?,?)";
    ...
    public int update(Student student) {
        con = ConnectionFactory.getConnection();
        try {
            String query = "update student set fname=?,lname=?,address=?,mobile_no=?,email_id=?,city=?, " + "designation=?,dob=?,doj=?,salary=? " + "where id=? ";
            ps = con.prepareStatement(query);
            ps.setString(1, student.getFname());
            ps.setString(2, student.getLname());
            ps.setString(3, student.getAddress());
    ...
    public Student fetchByName(String name){
        ...
        con = ConnectionFactory.getConnection();
        try {
            String query = "select * from student where fname = name";
            ps = con.prepareStatement(query);
            ps.setString(1, name);
            rs = ps.executeQuery();
            while (rs.next()) {
